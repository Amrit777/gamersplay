...
<div class="lightbox lightbox-user-gallery">
    <div class="row">
        @foreach ($userGallery as $galleryImage)
            <div class="col-lg-4 my-1" id="gallery-id-{{$galleryImage->id}}">
                ...
                <div class="likes gallery-reaction" 
                    data-gallery-image-id="{{ $galleryImage->id }}"
                    data-gallery-reaction-id="{{ $galleryImage->likedPost() }}">
                    <span class="gallery-like-heart heart float-left {{ $galleryImage->userliked() ? 'active-heart' : '' }}"><i
                            class="fas fa-heart"></i></span>
                    <span id="liked_gallery_count-{{ $galleryImage->id }}">{{ $galleryImage->likes->count() }}</span>
